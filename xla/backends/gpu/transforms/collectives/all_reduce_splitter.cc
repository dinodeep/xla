...
static std::optional<ARReplicaGroups> TryBuildSplitReplicaGroups(
    const HloAllReduceInstruction& ar, const HloDynamicSliceInstruction& ds,
    int split_dim, int ds_factor, int64_t num_partitions, int64_t num_replicas) {
...
static bool HasUniformReplicaGroupSizes(
    const std::vector<ReplicaGroup>& rgs, int64_t* group_size) {
